<?php
	$GLOBALS['mod_menu_array'] = parse_ini_file("conf.ini", true);
	echo("<script>cssLoad('module/mod_menu/css/mod_menu.css');</script>");
	echo("<script>$.getScript('module/mod_menu/js/mod_menu.js');</script>");
?>
				<input type="hidden" name="path" value="<?php echo($GLOBALS['ini_array']['path']); ?>" />
				<input type="hidden" name="componentPath" value="<?php echo($GLOBALS['ini_array']['componentPath']); ?>" />
				<input type="hidden" name="modulePath" value="<?php echo($GLOBALS['ini_array']['modulePath']); ?>" />
				<ul class="menu">
<?php
	foreach ($GLOBALS['mod_menu_array'] as $key => $value) {
		echo("<li>");
		echo($key);
